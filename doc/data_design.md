...
| `hasDueTime` | Boolean | 是否显式设置了截止时间；`false` 表示仅按日期截止，`true` 表示日期+时间 | 普通索引 |
...
3. 当前真实系统已稳定持久化 `hasDueTime`，避免把“仅日期任务”误解释为用户显式设置了 `00:00`。
4. 旧数据兼容策略：若记录中存在 `hasDueTime` 则以其为准；若历史记录缺失该字段，则默认按 `false` 处理，即视为“仅日期截止”。
...
- `hasDueTime=false` 时，`dueDate` 仍落为该日 `00:00 +08:00` 对应 UTC 时刻，但展示层必须以 `hasDueTime` 判断其语义是“仅日期截止”，不能回显为用户显式选择了 `00:00`。
